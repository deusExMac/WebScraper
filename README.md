...
All the rules specified in an exr file will be applied to the same downloaded web-page if the rule-specific conditions hold, before moving on to the next page which triggers again the application of the same library's rules. Each rule in a exr file is responsible for extracting only one specific kind of data (e.g. title, links, div content etc) from a downloaded page, if rule specific conditions hold. Rules return the extracted data as strings. One rule may return only one string value, return a list of string vaues or a list of objects.
