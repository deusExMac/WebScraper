...
Subdirectory rule/drafts contains unfinished, untested exr files that may be added as examples in future versions.
...
``webScraper.py [-W working directory path] [-c configuration file] [-r exr file] [-B] [-M] [-n number] [-s amount] [-o output] [-q queue] [-C] [-D] [-HR pct] [-CT regular expression[ [-R] [-ST] [-U] [-p pos] [-G] [-A] start_url``
...
-A: Appends to csv file, if it already exists.	
...
- #### crawl 	
  ...
  crawl starts the downloading of webpages and the extraction process based on the arguments given. The arguments of crawl are the same as WebScraper's command line arguments there were presented [above](#WebScraper-command-line-arguments). Example uses of the crawl command can be found [here](rules/index.html).  
...
- #### ps
  ...
  Syntax: ``ps [-process name regex]``
  
  [-process name regex] : Only processes whose name matches regular expression are shwon. If no argument is give, the OS specific chrome process is shown as specified by windowsChrome, macosChrome etc in the configuartion file.
